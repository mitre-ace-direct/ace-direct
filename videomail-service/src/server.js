...
const upload = require('./uploadS3.js');
...
function connectIncomingCall(ext, sdpOffer, callback) {
    ...
    getKurentoClient(function (error, kurentoClient) {
        ...
        kurentoClient.create('MediaPipeline', function (error, pipeline) {
            ...
            createMediaElements(ext, pipeline, function (error, rtpEndpoint, recorderEndpoint, playerEndpoint1, playerEndpoint2, playerEndpoint3, playerEndpointRec) {
                if (error) {
                    return callback(error);
                }
                ...
                rtpEvents(callee, rtpEndpoint, recorderEndpoint, playerEndpoint1, playerEndpoint2, playerEndpoint3);
                //Set Event Handlers for RecorderEndpoint
                recorderEvents(callee, recorderEndpoint, rtpEndpoint, playerEndpoint3, playerEndpointRec);

                // Call the function to connect the media components (endpoints).
                connectMediaElements(rtpEndpoint, recorderEndpoint, playerEndpoint1, playerEndpoint2, playerEndpoint3, function (error) {
                    if (error) {
                        pipeline.release();
                        return callback(error);
                    }
 ...
 */
function rtpEvents(callee, rtpEndpoint, recorderEndpoint, playerEndpoint1, playerEndpoint2, playerEndpoint3) {
    ...
    rtpEndpoint.on('ConnectionStateChanged', () => {
        if (playOnce) {
            ...
            setTimeout(function () {
                startPlayerEndpoint(playerEndpoint1);
            }, videoDelay);
        }
    });
    ...
    playerEndpoint2.on('EndOfStream', () => {
...
function createMediaElements(ext, pipeline, callback) {
    ...
    pipeline.create('RtpEndpoint', rtpParams, function (error, rtpEndpoint) {
        ...
        var recordExtension = (callee.incomingMediaProfile == 'MP4_VIDEO_ONLY') ? '.mp4' : '.webm';
        recordParams = {
            mediaPipeline: pipeline,
        ...
        pipeline.create('RecorderEndpoint', recordParams, function (error, recorderEndpoint) {
            ...
            var playerParams1 = {
                mediaPipeline: pipeline,
                uri: 'http://dev3demo.***REMOVED***.com:9321/' + config.playFileIntro1,
                useEncodedMedia: true
            }

            //PlayerEndpoint creation
            pipeline.create('PlayerEndpoint', playerParams1, function (error, playerEndpointIntro1) {
                if (error) {
                    debuglog('createMediaElements 3');
                    return callback(error);
                }
                log(callee.ext, "Playing from: " + playerParams1.uri);



                var playerParams2 = {
                    mediaPipeline: pipeline,
                    uri: 'http://dev3demo.***REMOVED***.com:9321/' + config.playFileIntro2,
                    useEncodedMedia: true
                }

                //PlayerEndpoint creation
                pipeline.create('PlayerEndpoint', playerParams2, function (error, playerEndpointIntro2) {
                    if (error) {
                        debuglog('createMediaElements 3b');
                        return callback(error);
                    }
                    log(callee.ext, "Playing from: " + playerParams2.uri);
                    ...
                    var playerParams3 = {
                        mediaPipeline: pipeline,
                        uri: 'http://dev3demo.***REMOVED***.com:9321/' + config.playFileIntro3,
                        useEncodedMedia: true
                    }

                    //PlayerEndpoint creation
                    pipeline.create('PlayerEndpoint', playerParams3, function (error, playerEndpointIntro3) {
                        if (error) {
                            debuglog('createMediaElements 3c');
                            return callback(error);
                        }
                        log(callee.ext, "Playing from: " + playerParams3.uri);
                        ...
                        playerParamsRec = {
                            mediaPipeline: pipeline,
                            uri: 'http://dev3demo.***REMOVED***.com:9321/' + config.playFileRec,
                            useEncodedMedia: true
                        }
                        pipeline.create('PlayerEndpoint', playerParamsRec, function (error, playerEndpointRec) {
                            ...
                            return callback(null, rtpEndpoint, recorderEndpoint, playerEndpointIntro1, playerEndpointIntro2, playerEndpointIntro3, playerEndpointRec);
                        });
 ...
 */
function connectMediaElements(rtpEndpoint, recorderEndpoint, playerEndpoint1, playerEndpoint2, playerEndpoint3, callback) {
    //rtpEndpoint.connect(recorderEndpoint, 'AUDIO');
    rtpEndpoint.connect(recorderEndpoint, 'VIDEO');
    //playerEndpoint1.connect(rtpEndpoint, 'AUDIO');
    playerEndpoint1.connect(rtpEndpoint, 'VIDEO');
