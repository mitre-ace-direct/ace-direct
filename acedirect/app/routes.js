...
if (config.filesharing && config.filesharing.enabled) {
  fileSharingEnabled = (utils.getConfigVal(config.filesharing.enabled) === 'true');
}
...
router.get('/token', restrict, (req, res) => {
  if (req.session.user.role === 'VRS') {
    ...
    if (/^\d+$/.test(vrsnum)) {
      utils.getCallerInfo(vrsnum, (vrsIn) => {
        const vrs = vrsIn;
        if (vrs.message === 'success') {
    ...
    utils.getUserInfo(username, (user) => {
      ...
      if (user.message === 'success') {
        req.redisClient.hget(c.R_STATUS_MAP, user.data[0].username, (_err, status) => {
          ...
          req.redisClient.hget(c.R_TOKEN_MAP, user.data[0].extension, (_errHget, tokenMapIn) => {
            let tokenMap = JSON.parse(tokenMapIn);
            const d = new Date();
            const now = d.getTime();
            ...
            req.session.user.queuesComplaintNumber = utils
              .getConfigVal(config.asterisk.queues.complaint.number); // why is this needed?
...
router.get('/getVideomail', agentRestrict, (req, res) => {
  ...
  // agentExt not used anywhere? Comment it out.
    ...
    if (err) {
      ...
      if (result[0].filepath === 's3') {
        ...
        const file = s3.getObject(
          { Bucket: utils.getConfigVal(config.s3.bucketname), Key: result[0].filename }
        );
...
router.get('/getRecording', agentRestrict, (req, res) => {
  ...
  const file = s3.getObject(
    { Bucket: utils.getConfigVal(config.s3.bucketname), Key: req.query.fileName }
  );
  ...
  if (uploadedBy) {
    ...
    if (utils.getConfigVal(config.filesharing.virus_scan_enabled) === 'true') {
      ClamScan.then(async (clamscan) => {
        try {
          console.log('scanning', uploadMetadata.filepath, 'as', os.userInfo().username, fs.existsSync(uploadMetadata.filepath));

          // You can re-use the `clamscan` object as many times as you want
          // const version = await clamscan.get_version();
          // console.log(`ClamAV Version: ${version}`);

          // const { isInfected, file, viruses } = await clamscan
          //   .is_infected(uploadMetadata.filepath);
          const { isInfected, viruses } = await clamscan.is_infected(uploadMetadata.filepath);
          if (isInfected) {
...
router.post('/videomailupload', consumerRestrict, (req, res) => { // add restrict. this is for testing only
  ...
  form.parse(req, (err, fields, files) => {
    if (err) {
      ...
      fs.readFile(filepath, (_err, fileData) => {
        if (files.file.mimetype === 'video/webm') {
          const uploadParams = { Bucket: utils.getConfigVal(config.s3.bucketname), Key: `${files.file.newFilename}.webm`, Body: '' };
          uploadParams.Body = fileData;
          s3.upload(uploadParams, (errS3Upload) => {
            if (errS3Upload) {
              console.log('Error!:', errS3Upload);
              return;
            }
            try {
              fs.unlinkSync(filepath);
            } catch (errUnlinkSync) {
              console.error(errUnlinkSync);
            }
