...
router.get('/profilePicPoll', (req, res) => {
  let profilePicFlag = { profilePicExists: false };
...
router.post('/videomailupload', consumerRestrict, (req, res) => { // add restrict. this is for testing only
  ...
  form.parse(req, (err, fields, files) => {
    if (err) {
      ...
      let filepath = files.file.filepath;
      fs.readFile(filepath, function (err, fileData) {
        if (files.file.mimetype === 'video/webm') {
          let uploadParams = { Bucket: utils.getConfigVal(config.s3.bucketname), Key: files.file.newFilename + '.webm', Body: '' };
