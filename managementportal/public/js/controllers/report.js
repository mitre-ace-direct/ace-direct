        ...
        if (data.message === "success") {
            ...
            socket.on('vrsreporttable-data', function (data) {
                if (data.message === "Success") {
                    ...
                    for (var i = 0; i < data.topTenStates.length; i++) {
                    ...
                    content = '<table style="width:100%"">';
                    for (var i = 0; i < data.topTenStates.length; i++) {
                        var vrsNumber = data.topTenVrsNumbers[i][0];
                        var vrsNumerFormatted = vrsNumber.substring(0, 3) + '-' + vrsNumber.substring(3, 6) + '-' + vrsNumber.substring(6, vrsNumber.length);
...
function updateCallStatusLineChart(data) {
    $(function () {
        if (data.data.length == 1) {
            var start = new Date(data.data[0].date);
            var end = new Date(start.getTime() + (24 * 60 * 60 * 1000));
            data.data[1] = Object.assign({}, data.data[0]);
