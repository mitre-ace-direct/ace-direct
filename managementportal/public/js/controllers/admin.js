...
angular.module('admin', ['csrService', 'datatables']).controller('adminController', ($scope, socket, DTOptionsBuilder) => {
  ...
  socket.on('forceLogoutPasswordNotPresent', () => {
  ...
  /**
     ...
     * we either add or remove the agent to the list of currently selected agents
     * within Angular's scope
     * */
  function updateSelected(action, tableAgent) {
    ...
    if (action === 'remove' && $scope.isSelected(tableAgent) === true) {
      ...
      for (let i = $scope.selected.length - 1; i >= 0; i -= 1) {
  ...
  /**
     * Updates the list of selected agents based on whether a row's checkbox
     * has been checked or unchecked
     */
  $scope.updateSelection = function UpdateSelection($event, tableAgent) {
  ...
  /**
     ...
     */
  $scope.getSelectedClass = function GetSelectedClass(tableAgent) {
  ...
  $('#forceLogoutModalButton').click((event) => {
    event.preventDefault();
    // Hide the modal
    $('#logoutConfirmationModal').modal('hide');
    // Get the agents to be logged out from the selected scope
    const agentsToBeLoggedOut = getSelectedAgentsForForcefulLogout();
